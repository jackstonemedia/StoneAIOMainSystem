...
import { 
  ...
  User, CheckSquare, Square, X, Table2, LayoutGrid, Columns, Eye, EyeOff
} from 'lucide-react';
...
const DEMO_CONTACTS: Contact[] = [
  { id: '1', name: 'Jack Stone', phone: '(408) 499-7331', email: 'jackxstonee@gmail.com', businessName: '', created: 'Apr 13, 2026 01:50 PM', lastActivity: '', tags: [], color: '#7dd3fc' },
  { id: '2', name: '(Example) Casey Morgan', phone: '+16541234567', email: '', businessName: '(Example) Dunder Miffl...', created: 'Apr 13, 2026 01:41 PM', lastActivity: '', tags: ['follow-up', '+1'], color: '#c4b5fd' },
  { id: '3', name: '(Example) Taylor Rey...', phone: '+178655689546', email: '', businessName: '(Example) MacLaren\'s...', created: 'Apr 13, 2026 01:41 PM', lastActivity: '', tags: ['follow-up'], color: '#93c5fd' },
  { id: '4', name: '(Example) Jordan S...', phone: '', email: 'jordan.smith@exampl...', businessName: '(Example) MacLaren\'s...', created: 'Apr 13, 2026 01:41 PM', lastActivity: '', tags: ['follow-up'], color: '#7dd3fc' },
  { id: '5', name: '(Example) Alex Doe', phone: '', email: 'alex.carter@business...', businessName: '(Example) Goliath Nati...', created: 'Apr 13, 2026 01:41 PM', lastActivity: '', tags: ['follow-up'], color: '#d1d5db' },
  { id: '6', name: '(Example) Riley Benn...', phone: '+13141236547', email: 'riley.bennett@corpor...', businessName: '(Example) Goliath Nati...', created: 'Apr 13, 2026 01:41 PM', lastActivity: '', tags: ['warm lead', '+1'], color: '#86efac' },
];
...
export default function Contacts() {
  ...
  // Panel states
  const [panelOpen, setPanelOpen] = useState<'filter' | 'manage' | 'new_contact' | null>(null);
    ...
    if (filters.length > 0) {
      result = result.filter(c => {
        const check = (f: typeof filters[0]) => {
          ...
          else if (f.field === 'created') fieldVal = c.created || '';
      ...
      <div className="px-8 flex items-center justify-between bg-surface z-10 sticky top-0 shadow-sm relative border-b border-border h-[68px]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <div className="flex items-center shadow-sm rounded-[4px] bg-surface relative">
              <button 
                onClick={() => setPanelOpen('new_contact')} 
                className="flex items-center gap-2 px-3 py-1.5 rounded-l-[4px] border border-border border-r-0 text-[14px] font-medium text-text-muted hover:text-text-main transition-colors active:scale-95"
              >
                <Plus className="w-4 h-4" /> Add Contact
              </button>
              <button 
                onClick={() => setAddContactDropdownOpen(!addContactDropdownOpen)}
                className="px-2 py-1.5 border border-border rounded-r-[4px] text-text-muted hover:text-text-main transition-colors flex items-center justify-center border-l bg-surface active:scale-95"
              >
              ...
              {addContactDropdownOpen && (
                <>
                  ...
                  >
                    ...
                    <button className="w-full flex items-center gap-3 px-4 py-2 text-[13px] font-medium text-text-muted hover:text-text-main hover:bg-surface-hover transition-colors">
        ...
        <table className="w-full text-left">
          <thead className="sticky top-0 z-10 border-b border-border/50 bg-surface/80 backdrop-blur-md shadow-sm">
            <tr>
              ...
              <th className="p-3 text-[13px] font-semibold whitespace-nowrap text-text-muted">
                <div className="flex items-center justify-between">Created (EDT) <ChevronDown className="w-3.5 h-3.5 opacity-40 hover:opacity-100 cursor-pointer transition-opacity" /></div>
              </th>
              <th className="p-3 text-[13px] font-semibold whitespace-nowrap text-text-muted">
                <div className="flex items-center justify-between">Last activity (EDT) <ChevronDown className="w-3.5 h-3.5 opacity-40 hover:opacity-100 cursor-pointer transition-opacity" /></div>
              </th>
          ...
          <tbody>
            {processedContacts.map((c) => (
              <tr key={c.id} className={`border-b border-border/50 transition-colors ${selected.has(c.id) ? 'bg-primary/5' : 'hover:bg-surface-hover/50'}`}>
                ...
                <td className="p-3">
                  <Link to={`/business/crm/contacts/${c.id}`} className="flex items-center gap-3">
                ...
                <td className="p-3 text-[13px] font-medium whitespace-nowrap text-text-main">{c.created}</td>
                <td className="p-3 text-[13px] font-medium whitespace-nowrap text-text-main">{c.lastActivity}</td>
                <td className="p-3">
            ...
            >
              <div className="px-6 py-5 flex items-center justify-between border-b border-border bg-surface-hover/50">
                <h2 className="text-[16px] font-bold text-text-main">
                  {panelOpen === 'filter' ? 'Advanced Filters' : panelOpen === 'manage' ? 'Manage Columns' : panelOpen === 'new_contact' ? 'Add Contact' : 'Create Smart List'}
                </h2>
              ...
              <div className="p-6 flex-1 overflow-auto bg-surface">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[12px] font-semibold text-text-muted uppercase tracking-wider">First Name</label>
                        <input type="text" placeholder="John" value={newContact.firstName} onChange={(e) => setNewContact({...newContact, firstName: e.target.value})} className="w-full px-3 py-2 bg-surface-hover border border-border rounded-[6px] text-[13px] text-text-main focus:outline-none focus:border-primary" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[12px] font-semibold text-text-muted uppercase tracking-wider">Last Name</label>
                        <input type="text" placeholder="Doe" value={newContact.lastName} onChange={(e) => setNewContact({...newContact, lastName: e.target.value})} className="w-full px-3 py-2 bg-surface-hover border border-border rounded-[6px] text-[13px] text-text-main focus:outline-none focus:border-primary" />
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[12px] font-semibold text-text-muted uppercase tracking-wider">Email</label>
                      <input type="email" placeholder="john@example.com" value={newContact.email} onChange={(e) => setNewContact({...newContact, email: e.target.value})} className="w-full px-3 py-2 bg-surface-hover border border-border rounded-[6px] text-[13px] text-text-main focus:outline-none focus:border-primary" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[12px] font-semibold text-text-muted uppercase tracking-wider">Phone</label>
                      <input type="tel" placeholder="+1 555-0000" value={newContact.phone} onChange={(e) => setNewContact({...newContact, phone: e.target.value})} className="w-full px-3 py-2 bg-surface-hover border border-border rounded-[6px] text-[13px] text-text-main focus:outline-none focus:border-primary" />
                    </div>
                    {activeListId !== 'all' && (
                ...
                {panelOpen === 'manage' && (
                  <div className="space-y-4">
                    {['Contact name', 'Phone', 'Email', 'Business name', 'Created (EDT)', 'Last activity (EDT)', 'Tags'].map(col => (
                      <div key={col} className="flex items-center gap-3 px-3 py-2.5 hover:bg-surface-hover rounded-[6px] transition-colors border border-transparent hover:border-border cursor-pointer">
                        <CheckSquare className="w-4 h-4 text-primary" />
                        <span className="text-[13px] font-medium text-text-main">{col}</span>
                      </div>
                    ))}
