...
function VoIPDialpad() {
  ...
  return (
    <div className="flex gap-6 h-full">
      ...
      {/* Call log */}
      <div className="flex-1 bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-[14px] font-bold text-text-main">Call Log</h2>
          <button className="text-[12px] font-semibold text-primary hover:underline">+ Log manual call</button>
        </div>
          ...
          {CALL_LOG.map(call => (
            <div key={call.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-surface-hover transition-colors">
              ...
              <div className="flex-1">
                <p className="font-semibold text-[13px] text-text-main">{call.name}</p>
                <p className="text-[11px] text-text-muted">{call.number} · {call.dir === 'in' ? '↙ Inbound' : '↗ Outbound'}</p>
              </div>
              <div className="text-right">
                <p className="text-[12px] font-semibold text-text-main">{call.duration}</p>
                <p className="text-[11px] text-text-muted">{call.time}</p>
              </div>
  ...
  return (
    <div className="flex gap-6 h-full">
            ...
            return (
              <div key={provider.name} className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[12px] font-semibold text-text-main">{provider.name}</p>
                    <p className="text-[10px] text-text-muted">{isConnected ? 'Synced 2 min ago' : 'Not connected'}</p>
                  </div>
                </div>
      ...
      <div className="flex-1 bg-surface border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-[14px] font-bold text-text-main">Inbox Threads</h2>
          <button className="text-text-muted hover:text-text-main"><RefreshCw className="w-4 h-4" /></button>
        </div>
            ...
            <div key={thread.id} className={`flex gap-4 px-6 py-4 cursor-pointer hover:bg-surface-hover transition-colors ${!thread.read ? 'bg-[#f0f7ff]' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span className={`text-[13px] ${!thread.read ? 'font-bold text-text-main' : 'font-semibold text-text-main'}`}>{thread.from}</span>
                  <span className="text-[11px] text-text-muted">{thread.time}</span>
                </div>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 220 }}
              ...
              <div className="p-4 space-y-3">
                <input value={to} onChange={e => setTo(e.target.value)} placeholder="To: email@company.com"
                  className="w-full px-3 py-2 border-b border-border text-[13px] text-text-main focus:outline-none placeholder:text-text-muted" />
                <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject"
                  className="w-full px-3 py-2 border-b border-border text-[13px] text-text-main focus:outline-none font-medium placeholder:text-text-muted" />
                <textarea value={body} onChange={e => setBody(e.target.value)} rows={7} placeholder="Write your message..."
                  className="w-full px-3 py-2 text-[13px] text-text-main focus:outline-none resize-none placeholder:text-text-muted leading-relaxed" />
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  <div className="flex items-center gap-2 text-text-muted">
                    <button className="hover:text-text-main transition-colors"><Bold className="w-4 h-4" /></button>
                    <button className="hover:text-text-main transition-colors"><Italic className="w-4 h-4" /></button>
                    <button className="hover:text-text-main transition-colors"><Link2 className="w-4 h-4" /></button>
...
function SMSTab() {
  ...
  return (
    <div className="flex h-full gap-0 bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
      {/* Thread list */}
      <div className="w-[240px] shrink-0 border-r border-border">
        <div className="px-4 py-3 border-b border-border"><h2 className="font-bold text-[13px] text-text-main">SMS Conversations</h2></div>
        <div className="divide-y divide-slate-100">
          {threads.map(t => (
  ...
  return (
    <div className="flex gap-6 h-full">
      <div className="flex-1 space-y-5">
        {/* Settings */}
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <h2 className="text-[14px] font-bold text-text-main mb-4">Meeting Settings</h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-[11px] font-bold text-text-muted uppercase tracking-wider block mb-1.5">Meeting Name</label>
              <input value={meetingName} onChange={e => setMeetingName(e.target.value)} className="w-full px-3 py-2 bg-surface-hover border border-border rounded-xl text-[13px] focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
            <div>
              <label className="text-[11px] font-bold text-text-muted uppercase tracking-wider block mb-1.5">Duration</label>
              <div className="flex gap-2">
                {[15, 30, 60].map(d => (
                  <button key={d} onClick={() => setDuration(d)}
                    className={`flex-1 py-2 rounded-xl text-[12px] font-bold border transition-colors ${duration === d ? 'bg-primary text-white border-primary' : 'border-border text-text-main hover:bg-surface-hover'}`}>
                    {d}m
                  </button>
                ))}
              </div>
        ...
        {/* Availability grid */}
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <h2 className="text-[14px] font-bold text-text-main mb-4">Availability <span className="text-[12px] font-normal text-text-muted">— click to toggle</span></h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th className="w-[90px]" />
                  {DAYS.map(d => <th key={d} className="text-[11px] font-bold text-text-muted pb-2">{d}</th>)}
                </tr>
              </thead>
              <tbody>
                {HOURS.map(hour => (
                  <tr key={hour}>
                    <td className="text-[11px] text-text-muted pr-3 py-0.5 text-right whitespace-nowrap">{hour}</td>
                    {DAYS.map(day => {
                      const slot = `${day}-${hour}`;
      ...
      <div className="w-[260px] shrink-0">
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <h2 className="text-[14px] font-bold text-text-main mb-4">Upcoming Meetings</h2>
          {UPCOMING.length === 0 ? (
            ...
            <div key={i} className="flex gap-3 py-3 border-b border-border last:border-0">
              ...
              <div>
                ...
                <p className="text-[11px] text-text-muted mt-0.5">{m.time}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        {platforms.map(p => (
          <a key={p.name} href={p.link} target="_blank" rel="noopener noreferrer"
            className="bg-surface border border-border rounded-2xl p-5 flex flex-col items-center gap-3 hover:border-primary/40 hover:shadow-md transition-all shadow-sm">
            <span className="text-3xl">{p.logo}</span>
            <span className="font-bold text-[14px] text-text-main">{p.name}</span>
            <span className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-xl text-[12px] font-semibold">
      ...
      <div className="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-border"><h2 className="font-bold text-[14px] text-text-main">Recent Video Calls</h2></div>
        <div className="divide-y divide-slate-100">
          {RECENT.map((r, i) => (
            <div key={i} className="px-6 py-4 flex gap-4">
              <Video className="w-5 h-5 text-text-muted shrink-0 mt-0.5" />
              <div>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="font-semibold text-[13px] text-text-main">{r.contact}</span>
                  <span className="text-[11px] text-text-muted">via {r.platform}</span>
                </div>
                <p className="text-[12px] text-text-muted">{r.notes}</p>
                <p className="text-[11px] text-text-muted mt-1">{r.duration} · {r.date}</p>
              </div>
            </div>
          ))}
        </div>
...
function SharedInboxTab() {
  ...
  return (
    <div className="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden h-full flex flex-col">
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        <h2 className="font-bold text-[14px] text-text-main">Shared Inbox</h2>
        <div className="flex items-center gap-2 text-text-muted">
        ...
        {THREADS.map(t => {
          ...
          return (
            <div key={t.id} className={`flex gap-4 px-6 py-4 hover:bg-surface-hover transition-colors cursor-pointer ${t.unread ? 'bg-[#f0f7ff]' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span className={`text-[13px] ${t.unread ? 'font-bold text-text-main' : 'font-semibold text-text-main'}`}>{t.from}</span>
                  <span className="text-[11px] text-text-muted">{t.time}</span>
                </div>
                <p className={`text-[12px] truncate ${t.unread ? 'font-semibold text-text-main' : 'text-text-main'}`}>{t.subject}</p>
                <p className="text-[11px] text-text-muted truncate">{t.preview}</p>
              </div>
              <div>
                <select value={assignees[t.id] || t.assignee || ''} onChange={e => setAssignees(prev => ({ ...prev, [t.id]: e.target.value }))}
                  className="text-[11px] font-semibold border border-border rounded-lg px-2 py-1 text-text-main focus:outline-none focus:ring-2 focus:ring-primary/30 bg-surface">
...
export default function Communications() {
  ...
  return (
    <div className="h-full flex flex-col bg-bg">
      {/* Header */}
      <header className="flex items-center justify-between px-8 py-5 border-b border-border bg-surface shrink-0">
        <div>
          <h1 className="text-[20px] font-bold text-text-main tracking-tight">Communications</h1>
          <p className="text-[13px] text-text-muted mt-0.5">Calls, email, SMS, meetings, video, and shared inbox — all in one place.</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-surface border border-border text-text-main rounded-xl font-semibold text-[13px] hover:bg-surface-hover transition-colors">
